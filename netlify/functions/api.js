...
app.use(session({
  ...
  cookie: { secure: true }, // Netlify는 HTTPS를 사용하므로 secure: true
...
router.get('/api/videos/metadata', isAuthenticated, async (req, res) => {
  try {
...
// 좋아요한 영상 가져오기
router.get('/api/liked-videos', isAuthenticated, async (req, res) => {
  try {
    // 페이지네이션 및 필터링 매개변수
